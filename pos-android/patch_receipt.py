...
with open('src/screens/ReceiptPreviewScreen.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# Find the start of shareReceiptWA and the end of the return block
share_func_start = content.find("    const shareReceiptWA = async () => {")
if share_func_start == -1:
    print("Could not find shareReceiptWA")
    sys.exit(1)

new_content = content[:share_func_start]

new_ui_and_share = """    const shareReceiptWA = async () => {
        try {
            // Capture image
    ...
    return (
        <View style={tw`flex-1 bg-gray-200 dark:bg-gray-900`}>
            {/* Header */}
            <View style={tw`bg-white dark:bg-gray-800 px-4 py-4 flex-row items-center border-b border-gray-300 dark:border-gray-700 shadow-sm z-10`}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={tw`mr-3`}>
                    <Icon name="arrow-left" size={24} color={tw.color('gray-600')} />
                </TouchableOpacity>
                <Text style={tw`text-xl font-bold text-gray-800 dark:text-gray-100 flex-1`}>Struk Transaksi</Text>
            </View>

            <ScrollView contentContainerStyle={tw`p-6 items-center`} showsVerticalScrollIndicator={false}>
                
                {/* The Paper Receipt inside ViewShot */}
                <ViewShot ref={viewShotRef} options={{ format: 'png', quality: 1.0 }} style={tw`bg-white w-full max-w-[320px] shadow-md mb-8`}>
                    <View style={tw`p-5`}>
                        {/* Logo */}
                        ...
                        <Text style={tw`text-center font-mono text-sm font-bold text-black mb-1`}>{settings.storeName || 'LITEPOS'}</Text>
...
with open('src/screens/ReceiptPreviewScreen.tsx', 'w', encoding='utf-8') as f:
    f.write(new_content + new_ui_and_share)

print("Patched ReceiptPreviewScreen.tsx")
